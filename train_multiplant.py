...
# --- Train/Val on multiple plants, test on one ---
CFG.multi_plant = args.multi_plant
...
for plant_root in [d for d in os.listdir("data") if os.path.isdir(os.path.join("data", d))]:
    ...
    # print(f"Log: Set dataset_root to {CFG.dataset_root}")
    ...
    for epoch in range(CFG.epochs):
        ...
        if (epoch + 1) % CFG.val_every == 0:
            ...
            if avg_val_loss < best_val_loss:
                ...
                no_improve_counter = 0
                checkpoint_path = os.path.join(ckpt_dir, f"predict_{dataset_name}_multiplant_{CFG.architecture}_best.pt")
                torch.save(
                    model.state_dict(),
                    checkpoint_path)
